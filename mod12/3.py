...
# Разработайте две функции для сериализации и десериализации списка контактов с помощью пакета csv 
# и хранения полученных данных в текстовом файле.
# Первая функция write_contacts_to_file принимает два параметра: filename — имя файла, 
# contacts — список контактов. Она сохраняет указанный список в файл формата csv.
...
# Вторая функция read_contacts_from_file читает, выполняет преобразование данных и возвращает 
# указанный список contacts из файла filename, сохраненного ранее функцией write_contacts_to_file.
# Примечание: При чтении csv файла мы получаем свойство словаря favorite в виде строки, т.е. 
# например favorite='False' . Необходимо его привести к логическому выражению обратно, чтобы стало
# favorite=False.
